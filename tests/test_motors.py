...
What it does:
  1. Motor 1 (theta) — servo sweeps 0° → 170° → 0°.
  2. Motor 2 (phi)   — stepper moves forward 16000 steps.
...
# ── Encoder 1 state (Motor 1 direction check only) ────────────────────────────
enc1_count = 0
_enc1_last_ms = 0.0
DEBOUNCE_MS = 3.0
...
def stepper_move(steps, label=""):
    print(f"  {label}: moving {steps} steps forward...")
...
def main():
    ...
    try:
        ...
        print("  Expected: servo sweeps 0° → 170° → 0°.")
        ...
        print(f"  Expected: head moves {M2_STEPS} steps along arc.")
